...
@click.option(
    '--version', help='the version of frkl you are using', is_flag=True)
@click.option(
    "--use-repo",
    "-r",
    required=False,
    multiple=True,
    help="extra context repos to use")
@click.pass_context
...
@cli.command('log')
@click.option(
    '--follow', '-f', help="follow the log file", is_flag=True, default=False)
@click.pass_context
def log_current(ctx, follow):
    ...
    last_run_folder = os.path.expanduser(
    ...
    if not follow:
        ...
        while True:
            ...
            if not f:
                f = subprocess.Popen(
                    ['tail', '-F', ansible_log_file], shell=False)
...
def debug_last(ctx):
    ...
    last_run_debug_script = os.path.join(last_run_debug_folder,
                                         "debug_all_plays.sh")
...
@click.option(
    '--filter',
    '-f',
    help="filters module names containing this string",
    required=False,
    default=None)
@click.option(
    '--details',
    '-d',
    help="print details about matching modules",
    required=False,
    default=False,
    is_flag=True)
@click.pass_context
    ...
    for m in module_list:
        ...
        try:
            ...
            if details:
                ...
                click.echo(": {}".format(
                    module_metadata["doc"]["short_description"]))

            click.echo("")
        except (ImportError):
            click.echo("{}: {}".format(
                short_name,
                "n/a (probably because of missing python dependencies)"))
...
@click.option(
    '--readme',
    '-r',
    help="print readme of matching roles",
    required=False,
    default=False,
    is_flag=True)
@click.option(
    '--defaults',
    '-d',
    help="print defaults file of matching roles",
    required=False,
    default=False,
    is_flag=True)
@click.option(
    '--meta',
    '-m',
    help="print meta details of matching roles",
    required=False,
    default=False,
    is_flag=True)
@click.pass_context
def list_roles_cli(ctx, filter, readme, defaults, meta):
    ...
    for repo in repos:
        for role, role_details in sorted(
            ...
            _author = role_details["meta"].get("galaxy_info", {}).get(
                "author", "n/a")
            _desc = role_details["meta"].get("galaxy_info", {}).get(
                "description", "n/a")
            ...
            if meta:
                ...
                yaml_string = yaml.dump(
                    role_details["meta"], default_flow_style=False)
...
@click.option(
    '--filter',
    '-f',
    help="filters blueprints names containing this string",
    required=False,
    default=None)
@click.option(
    '--details',
    '-d',
    help="print details about matching blueprints",
    required=False,
    default=False,
    is_flag=True)
@click.pass_context
...
@click.option(
    '--filter',
    '-f',
    help="filters adapters names containing this string",
    required=False,
    default=None)
@click.option(
    '--details',
    '-d',
    help="print details about matching adapters",
    required=False,
    default=False,
    is_flag=True)
@click.pass_context
def list_adapters_cli(ctx, filter, details):
    ...
    adapter_files = find_adapter_files(
        ADAPTER_MARKER_EXTENSION, adapters, config=config)
...
@click.option(
    "--filter",
    "-f",
    help="filter aliases contains the provided string",
    required=False,
    default=None)
@click.option(
    '--details',
    '-d',
    help="print alias details",
    required=False,
    default=False,
    is_flag=True)
@click.pass_context
def list_aliases_cli(ctx, filter, details):
    ...
    task_descs = calculate_task_descs(
        None, role_repos, add_upper_case_versions=False)
    ...
    for name, d in sorted(desc.items()):
        ...
        click.secho(
            ": {} {}".format(d["task_type"], d["task_name"]), bold=False)
        if details:
            click.echo("")
